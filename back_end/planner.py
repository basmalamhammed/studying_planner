...
def calculate_final_score(priority, urgency, focus):
    return priority + urgency + focus  # نضيف التركيز
...
def generate_weekly_schedule(subjects):
    ...
    day_index = 0
    for s in subjects:
        num_days = min(s["days"], 7)  # ما نتعداش الأسبوع
        # ساعات لكل يوم: scale 1-5 → 0.5-3 ساعات
